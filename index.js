...
// Direct test route browser ke liye
app.get('/test-ask', async (req, res) => {
    try {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                contents: [{ parts: [{ text: "Say hello JARVIS style" }] }]
            })
        });
...
app.post('/api/jarvis', async (req, res) => {
    try {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{ text: `You are JARVIS, Tony Stark's personal AI assistant. Reply briefly, sharply, and respectfully: ${prompt}` }]
                }]
            })
        });
